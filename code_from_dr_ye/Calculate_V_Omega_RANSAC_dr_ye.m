...
% global g_idxCam
global myCONFIG
if myCONFIG.FLAGS.GROUND_TRUTH_AVAILABLE
    [T,q,R]=get_gt_for_dynamic(stepPre,stepCurrent);
    varargout{1} = 0;
else
    ...
    if DebugFlag || myCONFIG.FLAGS.RECALCULATE || (~myCONFIG.FLAGS.RECALCULATE && ~exist(Dr_Ye_File,'file'))
        ...
        [file1, err]=sprintf('%s/d1_%04d.dat',myCONFIG.PATH.SOURCE_FOLDER,stepPre);
        [file2, err]=sprintf('%s/d1_%04d.dat',myCONFIG.PATH.SOURCE_FOLDER,stepCurrent);
        if DebugFlag
